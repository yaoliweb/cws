...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function setAllChecked() {
				var oTable = document.all['form1:content:adminTable'];
	...
	<f:view>
		<head>
			<title><h:outputText value="管理员配置" /></title>
		</head>
		<body>
			<center>
				<h:form id="form1">
					<h:inputHidden id="delDataIDStr"
						value="#{areaBean.delDataIDStr_admin}" />	
					<table>
						<tr>
												...
												<f:facet name="header">
													<h:panelGroup>
														<table width="98%">
															<tr>
																<td width="75%">
																	<h:outputText value="管理员配置" styleClass="panel-title"/>
																</td>
															</tr>
														</table>
													</h:panelGroup>
												</f:facet>
												<rich:dataTable id="adminTable" rows="16"
													value="#{areaBean.wraplistDataModel_admin}" var="admin"
													...
													<!-- ID -->
													 <rich:column >
		                                            	<h:outputText value="#{admin.administratorId}" style="display:none" />
													</rich:column>
													<!-- 名称 -->
													<rich:column sortBy="#{admin.name}">
														<f:facet name="header">
														    <h:outputText value="管理员"/>
														</f:facet>
														<h:commandLink action="#{areaBean.adminEditAction}">
															<h:outputText value="#{admin.name}"/>
															<f:param name="adminId" value="#{admin.administratorId}"/>
														</h:commandLink>
													</rich:column>																										
													...
													<rich:column>
														<f:facet name="header">
														    <h:outputText value="所属区县"/>
														</f:facet>
														<h:outputText value="#{admin.area.name}"/>
													</rich:column>
													...
													<tr>
														...
														<td width="6%" align="center">
															<h:commandButton value="#{msgs.delete}"
																...
																action="#{areaBean.deleteByIDs_admin}" style="height: 25px; width: 40px">
															</h:commandButton>
														</td>
														<td width="6%" align="center">
															<h:commandButton value="#{msgs.add}" action="#{areaBean.adminAddAction}"
														...
														<!-- 分页组件  -->
														<td width="75%">
												            <rich:datascroller for="adminTable" maxPages="10" align="right">
														        <f:facet name="first">
