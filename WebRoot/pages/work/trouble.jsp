...
<html>
	...
	<f:view>
		<head>
			<title><h:outputText value="污水站点故障列表" /></title>
		</head>
		<body>
			<center>
				<h:form id="form1">
												...
												<f:facet name="header">
													<h:panelGroup>
														<table >
															<tr>
																<td >
																	<h:outputText value="污水站点故障列表" styleClass="panel-title"/>
																</td>
															</tr>
														</table>
													</h:panelGroup>
									...
									<rich:panel style="width:800px;height:521px;" id="showcontent">
										<f:facet name="header">
												...
												<table>
													<tr>
														<td>
															<h:outputText value="#{sewageBean.areaSelectName}站点设备运行记录"
												...
												<rich:dataTable id="sewageTable" rows="16"
													...
													<rich:column>
														<f:facet name="header">
														    <h:outputText value="曝气机"/>
														</f:facet>
														<h:outputText value="#{sewage.equipment1state eq '1'? '运行':(sewage.equipment1state eq '2'?'停止':(sewage.equipment1state eq '3'?'故障':'未安装'))}"/>
													</rich:column>
													<!-- 污水泵 -->
													<rich:column>
														<f:facet name="header">
														...
														<h:outputText value="#{sewage.equipment2state eq '1'? '运行':(sewage.equipment2state eq '2'?'停止':(sewage.equipment2state eq '3'?'故障':'未安装'))}"/>
													</rich:column>
											<!-- 回流泵 -->
												<rich:column>
														<f:facet name="header">
														    <h:outputText value="回流泵"/>
														</f:facet>
														<h:outputText value="#{sewage.equipment3state eq '1'? '运行':(sewage.equipment3state eq '2'?'停止':(sewage.equipment3state eq '3'?'故障':'未安装'))}"/>
													</rich:column>
											<!-- 吸膜泵 -->
													<rich:column>
														<f:facet name="header">
														...
														<h:outputText value="#{sewageBean.selectSewageControlId eq '1'? '未安装':(sewageBean.selectSewageControlId eq '3'?'未安装':(sewage.equipment4state eq '1'?'运行':sewage.equipment4state eq '2'?'停止':sewage.equipment4state eq '3'?'故障':'未安装'))}"/>
													</rich:column>
												</rich:dataTable>
												<br/>
