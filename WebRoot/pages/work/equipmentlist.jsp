								...
								<f:subview id="functioncontentpage">
									<rich:panel style="width:800px;height:65px;">
										...
										<a4j:commandButton value="#{msgs.search}"
											reRender="showcontent,runDataTable"></a4j:commandButton>
									</rich:panel>
									<rich:panel style="width:800px;height:521px;" id="showcontent">
										<f:facet name="header">
											<h:panelGroup>
												<table>
													<tr>
														<td>
															<h:outputText value="#{sewageBean.sewageSelectName}站点设备运行记录"
																styleClass="panel-title"></h:outputText>
														</td>
													</tr>
												</table>
										...
										<rich:dataTable id="runDataTable"
											...
											<rich:column>
												<f:facet name="header">
													<h:outputText value="#{sewageBean.selectSewageControlId eq '1'? '曝气机':(sewageBean.selectSewageControlId eq '2'?'风机':(sewageBean.selectSewageControlId eq '3'?'电磁式气泵':'曝气机'))}" />
												</f:facet>
												<h:outputText
											...
											<rich:column>
												<f:facet name="header">
													<h:outputText value="#{sewageBean.selectSewageControlId eq '1'? '污水泵':(sewageBean.selectSewageControlId eq '2'?'提升泵':(sewageBean.selectSewageControlId eq '3'?'潜水提升泵':'污水泵'))}" />
												</f:facet>
												<h:outputText
											...
											<rich:column>
												<f:facet name="header">
													<h:outputText value="#{sewageBean.selectSewageControlId eq '1'? '回流泵':(sewageBean.selectSewageControlId eq '2'?'洗膜泵':(sewageBean.selectSewageControlId eq '3'?'洗潜水排水泵':'回流泵'))}" />
												</f:facet>
												<h:outputText
											...
											<rich:column>
												...
												<h:outputText value="排水泵" />
												</f:facet>
												<h:outputText
													value="#{sewageBean.selectSewageControlId eq '1'? '未安装':(sewageBean.selectSewageControlId eq '3'?'未安装':(runData.equipment4state eq '1'?'运行':runData.equipment4state eq '2'?'停止':runData.equipment4state eq '3'?'故障':'未安装'))}" />
											</rich:column>
											<rich:column>
												<f:facet name="header">
												...
												<h:outputText
													value="#{runData.equipment5state eq '1'? '高':(runData.equipment5state eq '0'?'低':'异常')}" />
											</rich:column>
										</rich:dataTable>
										<br/>
